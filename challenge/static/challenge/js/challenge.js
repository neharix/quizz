...
let confirmation = JSON.parse(
  document.querySelector("#confirmation").textContent
);
let question_id = JSON.parse(
  document.querySelector("#question-id").textContent
);
...
answers.forEach((element) => {
  element.onclick = (e) => {
    if (selected_answer != findAncestor(e.target, ".answer")) {
      selected_answer = findAncestor(e.target, ".answer");
      selected_answer.children[0].children[0].checked = true;
      answers.forEach((element) => {
      ...
      findAncestor(e.target, ".answer").children[0].children[0].checked = false;
      findAncestor(e.target, ".answer").classList.remove("selected");
    }
  };
});
